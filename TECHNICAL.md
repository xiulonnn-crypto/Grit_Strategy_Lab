...
- 二期 target 现在必须执行真实 provider 落库路径，而不是只回显已有 coverage：`fundamentals` 写入 FMP statement points / coverage，`sentiment` 写入 Alpha Vantage earnings expectation 与 FINRA short-volume signal points / coverage，`macro_derivatives` 写入 FRED macro rates 与 Massive/Polygon option-skew signal points / coverage。provider 没有凭据、限流、权限不足或返回缺少时间凭证时，只能进入 `latest_job.summary.refresh_stats.datasets[*].provider_summary`、provider-attempt/readiness evidence 与质量告警，不得创建正式 `dataset_snapshots[]` row。
...
- L4 phase2 默认 provider 基线是 Massive 精修链路，而不是 ThetaData。`MASSIVE_API_KEY` 作为 option-skew / precision evidence 的唯一凭据来源；ThetaData 后续如接入，只能作为可选第二适配器，不得反向改写本次 phase2 的主链路。
