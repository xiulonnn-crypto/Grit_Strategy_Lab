...
- For `#/factors` family/tag/filter incidents, run the route probe before manual browser work: `powershell -ExecutionPolicy Bypass -File .\scripts\codex-probe-factor-family-route.ps1 -FactorId <id> -ExpectedFamily <family> -RejectFamily <bad-family> -Browser -StartPreview -Strict -Json`. It records runtime preflight, `/factors?lifecycle=all` family rows, scoped factor-family filter checks, and a route screenshot under `output/logs/grit-coder/<task-slug>/`.
- For public factor import naming acceptance on `#/factors/imports`, prefer `powershell -ExecutionPolicy Bypass -File .\scripts\codex-probe-factor-imports-name.ps1 -JobId <job-id> -FactorId <source-id> -ExpectedName <cn-name> -RejectName <old-name> -Strict -Json`. It records runtime preflight, `/factor-sources/import-jobs/{id}`, `/factor-factory/overview`, the factory-only naming probe, and optional browser screenshot evidence under `output/logs/grit-coder/<task-slug>/`.
...
- SEC 424B2 runtime preflight 可显式传入 `initial_value_proxy_mode=SANDBOX_ONLY_FIRST_PRICE`，仅在 `replay_mode=sandbox` 下用每个底层资产窗口内首个 `dataset_price_bars.adj_close` 临时补齐缺失 `initial_value`；该值只存在于 preflight 内存副本和审计证据中，不写回 `structured_note_underlyings`，响应必须标记 `SANDBOX_ONLY_INITIAL_VALUE_PROXY_USED` 并保持 `preflight_only=true`。
...
- SEC 424B2 Phase 1.9 将 10 条 Clean Preflight 后的影子回放与全量导入拆开：正式持久化单票据回放仍以 `POST /structured-notes/fcn/replay` 为唯一 canonical route，`POST /structured-notes/fcn/replay/run` 只是兼容 alias；批量影子回放使用 `POST /structured-notes/fcn/replay/batch`，写 `structured_note_replay_batches` 审计行和每只 note 的正式 replay run / replay points，但不写 Factor Factory 或正式因子库。批量归因聚合由 `GET /structured-notes/fcn/replay-batches/{batch_id}/attribution-summary` 读取已有 replay run 结果，不重新拉价格。
- SEC 424B2 全量 Big Load 使用 `POST /structured-notes/sec-424b2/ingestion-jobs/preview`、`POST /structured-notes/sec-424b2/ingestion-jobs` 与 `GET /structured-notes/sec-424b2/ingestion-jobs/{job_id}`；默认 dry-run 只生成 accession/action 审计，`apply_append_only` 只追加新 parse run 并按 accession/parser/version/hash 幂等跳过既有结果。`GRIT_SEC_424B2_RATE_LIMIT_RPS` 默认按 `5` 记录并用于实际 SEC fetch 节流，ingestion job 不触发 replay、quarantine、publish 或仓位修改。
- SEC 424B2 Factor Factory 挂载先只暴露合同：`GET /structured-notes/fcn/factor-factory-mount-contract` 返回稳定 F2 definition ids、`note_id + note_date` 维度、中文展示名和 `BLOCKED_UNTIL_QUARANTINE_PUBLISH` 写入状态。10,000+ note 是实例序列，不允许生成 10,000+ 个 factor definition；晋升 `[外部] - FCN增强收益归因 [精炼]` 必须在后续通过 Factor Factory / D2 Quarantine / publish 治理。
