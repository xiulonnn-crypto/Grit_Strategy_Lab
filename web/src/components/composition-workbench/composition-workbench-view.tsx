...
function getCorrelationToneClass(value: number): string {
  if (value >= 0.7) {
    return 'composition-workbench-correlation-cell--high';
  }
  if (value <= -0.2) {
    return 'composition-workbench-correlation-cell--inverse';
  }
...
export function CompositionWorkbenchView({
  ...
  const chartSeries = preview ? preview.returns_preview.map((item) => item.cumulative_return_pct) : [];
  ...
  const previewRebalanceSummary = preview ? preview.rebalance_summary : null;
    ...
    >
      ...
      <div className="composition-workbench-hero__chips">
        <span className="composition-workbench-chip composition-workbench-chip--accent">
          来源 {joinedCount} / {Math.max(joinedCount, availableCount)} 已加入
        </span>
      ...
      <div className="composition-workbench-main-grid">
          ...
          <div className="composition-workbench-source-list">
            {loading ? (
              ...
              filteredRows.map((row) => {
                ...
                return (
                  <article className="composition-workbench-source-card" key={row.id}>
          ...
          <div className="composition-workbench-structure-list">
            {selectedPreviewLegs.length === 0 ? (
              ...
              selectedPreviewLegs.map(({ draft, preview: previewLeg }) => {
                ...
                return (
                  <article
                    className={highlighted ? 'composition-workbench-leg-card is-highlighted' : 'composition-workbench-leg-card'}
        ...
        <aside className="panel composition-workbench-summary-rail">
          ...
          <label className="composition-workbench-field">
            <span>组合名称</span>
            <input
          ...
          <div className="composition-workbench-score">
            <button
              className="composition-workbench-score__ring"
              onClick={() => setScoreOpen((current) => !current)}
              style={{
                background: `conic-gradient(#1f877b 0deg ${(previewScore?.score ?? 0) * 3.6}deg, rgba(148, 163, 184, 0.16) ${(previewScore?.score ?? 0) * 3.6}deg 360deg)`,
              }}
              type="button"
            >
              <span>
                <strong>{formatRatio(previewScore?.score ?? 0)}</strong>
                <small>成立性评分</small>
              </span>
            </button>
            <div className="composition-workbench-score__copy">
              <strong>{previewVerdict}</strong>
              <p>基于最近 24 个月收益流代理与当前协方差矩阵计算。</p>
            </div>
          </div>

          {scoreOpen && previewFactors.length ? (
            <div className="composition-workbench-score-breakdown">
              {previewFactors.map((factor) => (
                <article className="composition-workbench-score-factor" key={factor.key}>
                  <div className="composition-workbench-score-factor__head">
                    <strong>{formatCompositionFactorLabel(factor.key, factor.label)}</strong>
                    <span>{formatRatio(factor.score)}</span>
                  </div>
                  <div className="composition-workbench-score-factor__bar">
                    <span style={{ width: `${Math.min(factor.score, 100)}%` }} />
                  </div>
                  <p>{formatCompositionFactorDetail(factor.key, factor.detail)}</p>
                </article>
              ))}
            </div>
          ) : null}
          ...
          <label className="composition-workbench-field">
            <span>再平衡频次</span>
            <select
              onChange={(event) => onRebalanceFrequencyChange(event.target.value)}
          ...
          <div className="composition-workbench-diagnostics">
            <article className="composition-workbench-diagnostic-card">
              <strong>权重校验</strong>
              <p>
                当前总权重 {previewWeightSummary ? previewWeightSummary.total_weight_pct.toFixed(1) : '0.0'}% ·
                残余 {previewWeightSummary ? previewWeightSummary.residual_weight_pct.toFixed(1) : '0.0'}%
              </p>
            </article>
            <article className="composition-workbench-diagnostic-card">
              <strong>维护成本</strong>
              <p>
                预计总维护成本 {previewMaintenanceSummary?.total_estimated_bps ?? 0} bps ·
                {getCadenceLabel(previewRebalanceSummary?.rebalance_frequency ?? rebalanceFrequency)}
              </p>
            </article>
          </div>
          ...
          <div className="composition-workbench-correlation-alerts">
            <div className="composition-workbench-correlation-alerts__head">
              <strong>相关性提醒</strong>
              <span>{correlationInsights.length} 条</span>
            </div>
            {correlationInsights.length ? (
              correlationInsights.map((item) => (
                <button
                  className={highlightedPair === item.key ? 'composition-workbench-correlation-alert is-active' : 'composition-workbench-correlation-alert'}
                  key={item.key}
                  onClick={() => setHighlightedPair((current) => (current === item.key ? null : item.key))}
                  type="button"
                >
                  <div>
                    <strong>
                    ...
                    <p>{getCorrelationPairLabel(item.correlation)}</p>
                  </div>
                  <span
                    className={item.tone === 'warning' ? 'status-chip status-chip--warning' : 'status-chip status-chip--soft'}
                  >
          ...
          <div className="composition-workbench-save-dock">
            <button
              className="ghost-button"
              disabled={saving || selectedLegs.length === 0}
              onClick={() => {
        ...
        <section className="panel composition-workbench-panel">
          <div className="panel-header composition-workbench-panel__header">
            <div>
              <h2>收益流预览</h2>
              <p className="composition-workbench-panel__copy">
                实线表示组合累计收益，虚线表示基准。这里只做最后确认，不抢首屏的操作叙事。
              </p>
            </div>
            <span className="status-chip status-chip--soft">
              基准 {resolvedBenchmarkLabel}
