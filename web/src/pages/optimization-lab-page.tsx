...
type HeatmapMetricKey = 'annualized_return' | 'return_sharpe' | 'max_drawdown_pct';
type OptimizationDisplayCandidate = ApiOptimizationCandidate & {
  display_kind?: 'candidate' | 'baseline';
};

const OPTIMIZATION_POLL_INTERVAL_MS = import.meta.env.MODE === 'test' ? 50 : 3000;
...
const TEXT = {
  ...
  jobsDeleteCopy: '逻辑删除后，该任务会从列表与详情页隐藏；若后台计算已经启动，本次删除不会中断正在执行的计算进程。',
  jobsDeleting: '正在删除...',
  cancel: '取消',
  jobsTitle: '优化任务列表',
...
const PANEL_SUBTITLES = {
  candidatePanel: '按综合评分排序复核本轮参数组合，优先比较年化收益、样本外表现与回撤约束，再决定是否进入晋升评估。',
  stabilityCenter: '聚焦首选参数的风险收益平衡与稳健性证据，用于判断其是否具备晋升为正式参数版本的条件。',
  heatmapPanel: '观察关键参数在局部搜索空间内的绩效梯度，识别高收益高回撤陷阱与更稳健的甜区。',
  validationPanel: '按不同市场窗口复核策略表现，重点检查年化收益延续性、样本外衰减与回撤漂移。',
} as const;
...
function translateOptimizationText(value?: string | null): string | null {
  ...
  const exactReplacements: Array<[RegExp, string]> = [
    ...
    [/^Resuming optimization from trial\s+(\d+)\.?$/i, '\u6b63\u5728\u4ece\u7b2c $1 \u7ec4\u7ee7\u7eed\u4f18\u5316\u3002'],
    ...
    [/^Progress preserved at\s+(\d+)\/(\d+)\.\s*Click Continue Optimization to resume\.?$/i, '已保留 $1/$2 的进度，点击“继续优化”即可恢复。'],
...
function getCandidateDisplayText(value: string | null | undefined, fallback = '-'): string {
  const repaired = repairMojibakeText(value);
...
function getSearchFieldDisplayLabel(field: Pick<ApiOptimizationSearchSpaceField, 'key' | 'label'>): string {
  const translatedLabel = translateOptimizationText(field.label);
  if (!translatedLabel || translatedLabel === field.key || isMostlyAsciiLabel(translatedLabel)) {
...
function calculateElapsedSeconds(startAt?: string | null, endAt?: string | null): number | null {
  if (!startAt || !endAt) {
    return null;
  }
...
function formatVersionedStrategyName(name: string | null | undefined, version: number | undefined): string {
  const baseName = stripStrategyVersionSuffix(String(name ?? '').trim()) || String(name ?? '').trim() || TEXT.resultsTitle;
...
function formatOptimizationHeroSummary(combinations: number | undefined, elapsedSeconds: number | null): string | null {
  const combinationCount =
    typeof combinations === 'number' && Number.isFinite(combinations) && combinations > 0 ? Math.round(combinations) : null;
...
function buildDerivedValidationWindows(metrics: Record<string, number>): ApiOptimizationValidationWindow[] {
  const annualizedReturn = readNumber(metrics.annualized_return ?? metrics.cagr) ?? 0;
  const returnSharpe = readNumber(metrics.return_sharpe ?? metrics.sharpe) ?? 0;
  const outOfSampleSharpe = readNumber(metrics.out_of_sample_sharpe) ?? returnSharpe;
  const maxDrawdownPct = readNumber(metrics.max_drawdown_pct) ?? 0;
  const stability = readNumber(metrics.stability) ?? 0;
  const baseVerdict = annualizedReturn >= 0.1 && returnSharpe >= 1 ? 'pass' : annualizedReturn >= 0.06 ? 'watch' : 'risk';
  ...
  return [
    ...
    },
    {
      ...
      verdict: annualizedReturn >= 0.08 && outOfSampleSharpe >= 0.5 ? 'watch' : 'risk',
    },
  ];
}
...
function buildDerivedStabilityChecks(
  ...
  return [
    ...
    },
    {
      ...
      verdict: returnSharpe >= 1 ? 'pass' : returnSharpe >= 0.6 ? 'watch' : 'risk',
      detail: returnSharpe >= 1 ? '收益风险比处于可接受区间。' : '收益风险比仍需与优化候选对照观察。',
    },
    {
      key: 'out_of_sample_sharpe',
      label: '样本外夏普',
      value: Number(outOfSampleSharpe.toFixed(2)),
      verdict: outOfSampleSharpe >= 0.8 ? 'pass' : outOfSampleSharpe >= 0.4 ? 'watch' : 'risk',
      detail: outOfSampleSharpe >= 0.8 ? '样本外表现具备延续性。' : '样本外延续性仍需和优化候选比较。',
    },
    {
      key: 'max_drawdown_pct',
      label: '最大回撤',
      value: Number(maxDrawdownPct.toFixed(1)),
      verdict: maxDrawdownPct >= -25 ? 'pass' : maxDrawdownPct >= -35 ? 'watch' : 'risk',
      detail: maxDrawdownPct >= -25 ? '当前回撤仍在风控护栏内。' : '当前回撤需要用优化候选进一步改善。',
    },
    {
      key: 'stability',
      label: '稳定度',
      ...
      detail: stability >= 70 ? '参数组合具备稳定对照价值。' : '稳定度仍需通过优化候选提升。',
    },
  ];
}
...
function buildSyntheticHeatmap(
  ...
  const xCenter = readNumber(parameterSnapshot?.[xEntry.key] ?? xEntry.current) ?? 0;
  const yCenter = readNumber(parameterSnapshot?.[yEntry?.key ?? ''] ?? yEntry?.current) ?? xCenter;
  ...
  const centerAnnualizedReturn = readNumber(metrics.annualized_return ?? metrics.cagr) ?? 0;
  const centerReturnSharpe = readNumber(metrics.return_sharpe ?? metrics.sharpe) ?? 0;
    ...
    cells: yValues.flatMap((yValue, rowIndex) =>
      xValues.map((xValue, columnIndex) => {
        ...
        return {
          ...
          score,
          metrics: {
            annualized_return: Number(clampMetric(centerAnnualizedReturn - distance * 0.008, -0.99, 9.99).toFixed(4)),
  ...
  return {
    ...
    cells: heatmap.cells.map((cell) => {
      ...
      return {
        ...cell,
        metrics: {
          annualized_return: Number(clampMetric(centerAnnualizedReturn - xDistance * 0.004 - yDistance * 0.003, -0.99, 9.99).toFixed(4)),
          return_sharpe: Number((centerReturnSharpe - xDistance * 0.03 - yDistance * 0.02).toFixed(3)),
          max_drawdown_pct: Number((centerMaxDrawdownPct - xDistance * 1.4 - yDistance * 1.1).toFixed(1)),
        },
      };
    }),
  };
...
function formatHeatmapMetricValue(metricKey: HeatmapMetricKey, value: number | undefined): string {
  if (typeof value !== 'number' || Number.isNaN(value)) {
    return '-';
  }
...
function readProgressBoolean(source: Record<string, unknown> | undefined, key: string, fallback = false): boolean {
  const value = source?.[key];
  if (typeof value === 'boolean') {
    return value;
  }
...
function isOptimizationProgressState(status?: string | null): boolean {
  return ['QUEUED', 'RUNNING', 'INTERRUPTED'].includes(String(status ?? '').toUpperCase());
}

function formatBestMetricsSummary(summary: ApiOptimizationTrialSummary | undefined | null): string | null {
  if (!summary) {
    return null;
  }
  ...
  const outOfSampleSharpe =
    typeof metrics.out_of_sample_sharpe === 'number' ? metrics.out_of_sample_sharpe : undefined;
  const maxDrawdownPct =
    typeof metrics.max_drawdown_pct === 'number'
      ...
      : typeof metrics.max_drawdown === 'number'
        ...
        : undefined;
  const parts = [
    typeof annualizedReturn === 'number' ? `年化 ${formatReturnRate(annualizedReturn)}` : null,
    typeof sharpe === 'number' ? `收益夏普 ${formatMetric(sharpe)}` : null,
    typeof outOfSampleSharpe === 'number' ? `样本外 ${formatMetric(outOfSampleSharpe)}` : null,
    typeof maxDrawdownPct === 'number' ? `回撤 ${formatPercentMetric(maxDrawdownPct)}` : null,
  ].filter((value): value is string => Boolean(value));
  return parts.length ? parts.join(' · ') : null;
}

function describeCandidateEvaluation(candidate: OptimizationDisplayCandidate | null): string {
  if (!candidate) {
    return '等待候选结果。';
  }

  const annualizedReturn = getCandidateMetric(candidate, 'annualized_return') ?? getCandidateMetric(candidate, 'cagr');
  ...
  const verdict = translateOptimizationText(candidate.status_label) ?? formatOptimizationStatus(candidate.status);
...
function getCandidateMetric(candidate: OptimizationDisplayCandidate, key: string): number | undefined {
...
function buildBaselineCandidate(
  ...
  const returnSharpe = readNumber(latestCompletedRun?.sharpe) ?? readNumber(baselineRun.metrics?.sharpe);
  const outOfSampleSharpe =
  ...
  const stability = clampMetric(
    Math.round(((outOfSampleSharpe ?? returnSharpe ?? 0) * 32 + (annualizedReturn ?? 0) * 180 - Math.abs(maxDrawdownPct ?? 0) * 0.35)),
    35,
    88,
  );

  if (annualizedReturn === undefined && returnSharpe === undefined && maxDrawdownPct === undefined) {
    return null;
  }
  ...
  const heatmap = buildSyntheticHeatmap(searchSpace, parameterSnapshot, metrics);
  ...
  return {
    ...
    analysis: {
      title: label,
      thesis: '当前策略组合仅作为对照基准，用于观察优化候选相对现行参数在收益、样本外延续性与回撤约束上的改善幅度。',
      shelf_copy: summary,
      stability_verdict: '当前基准',
      stability_summary: '当前策略组合用于与优化候选横向比较，重点观察年化收益、样本外夏普与回撤改善幅度。',
...
function OptimizationStepBar({
  ...
  const steps: Array<{ key: 'select' | 'config' | 'results'; title: string; description: string; href?: string }> = [
...
export function OptimizationJobsIndexPage(): JSX.Element {
  ...
  const [jobPendingDelete, setJobPendingDelete] = useState<ApiOptimizationJobListItem | null>(null);
  ...
  async function confirmDeleteJob(): Promise<void> {
    ...
    try {
      ...
      setJobs((currentJobs) => currentJobs.filter((job) => job.id !== jobPendingDelete.id));
      setJobPendingDelete(null);
    } catch (caught) {
    ...
    <div className="optimization-lab-page">
              ...
              <tbody>
                {jobs.map((job) => (
                  <tr key={job.id}>
                    ...
                    <td>{getDisplayText(job.strategy_name, job.strategy_id)}</td>
                    <td>{formatValidationMode(job.validation_mode)}</td>
                    <td>
                      {job.completed_combinations ?? 0} / {job.budget_combinations ?? 0}
                    </td>
                    <td>{getCandidateDisplayText(job.best_candidate_label, '-')}</td>
                    ...
                    <td className="optimization-jobs-table__actions">
                      <button
                        ...
                        type="button"
                      >
                        {deleteBusy && jobPendingDelete?.id === job.id ? TEXT.jobsDeleting : TEXT.jobsDeleteAction}
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      {jobPendingDelete ? (
        <div
          ...
          role="dialog"
        >
          <div className="modal-card optimization-jobs-delete-dialog" onClick={(event) => event.stopPropagation()}>
    ...
    <div className="optimization-lab-page">
      ...
      <section className="optimization-lab-panel optimization-lab-panel--hero">
        <div>
          ...
          <h1>{translateOptimizationText(strategy?.name) ?? TEXT.configTitle}</h1>
          <p>{TEXT.configCopy}</p>
          {!loading && strategy ? (
                ...
                <tbody>
                  {searchSpace.map((field, index) => (
                    <tr key={field.key}>
                      ...
                      <td>{getDisplayText(field.tag, '-')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
export function OptimizationResultsPage({ jobId }: { jobId: string }): JSX.Element {
  ...
  const [strategy, setStrategy] = useState<ApiStrategyDetail | null>(null);
  const [baselineRun, setBaselineRun] = useState<ApiBacktestRunDetail | null>(null);
  const [selectedCandidateId, setSelectedCandidateId] = useState<string | null>(null);
  const [selectedHeatmapMetric, setSelectedHeatmapMetric] = useState<HeatmapMetricKey>('annualized_return');
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      try {
        ...
        const baselineRunId =
          (typeof jobPayload.request.source_run_id === 'string' && jobPayload.request.source_run_id.trim()
            ? jobPayload.request.source_run_id
            : strategyPayload.latest_completed_run_summary?.run_id ?? strategyPayload.latest_run_id) ?? null;
        if (baselineRunId) {
          try {
  ...
  const baselineCandidate = useMemo(
    () =>
      job?.candidates.length
        ? buildBaselineCandidate(strategy, baselineRun, optimizationSearchSpace, (job.candidates.length ?? 0) + 1)
        : null,
  ...
  const candidateRows = useMemo<OptimizationDisplayCandidate[]>(
    () => [...(job?.candidates ?? []), ...(baselineCandidate ? [baselineCandidate] : [])],
  ...
  useEffect(() => {
    ...
    const fallbackCandidateId = job.result.best_candidate_id ?? candidateRows[0]?.id ?? null;
    if (!selectedCandidateId || !candidateRows.some((candidate) => candidate.id === selectedCandidateId)) {
  ...
  const optimizationInterrupted = String(job?.status ?? '').toUpperCase() === 'INTERRUPTED';
  const progressSummary = (job?.summary ?? {}) as Record<string, unknown>;
  const progressResult = (job?.result ?? {}) as Record<string, unknown>;
  const progressPct = readProgressNumber(progressSummary, 'progress_pct', optimizationProgressState ? 0 : 100);
  ...
  const optimizationCombinationCount = budgetCombinations || completedCombinations || undefined;
  const resumeReady = readProgressBoolean(progressSummary, 'resume_ready', true);
  const persistedTrialCount = readProgressNumber(progressSummary, 'persisted_trial_count', completedCombinations);
  const nextTrialIndex = readProgressNumber(progressSummary, 'next_trial_index', completedCombinations + 1);
  ...
  const interruptedReason = formatInterruptedReason(readProgressText(progressSummary, 'interrupted_reason')) ??
    (optimizationInterrupted ? '优化任务已中断，当前进度已保存。' : null);
  const bestMetricsSummary = formatBestMetricsSummary(job?.summary.best_metrics_summary);
  const currentStage =
    translateOptimizationText(readProgressText(progressSummary, 'current_stage')) ??
    translateOptimizationText(readProgressText(progressResult, 'current_stage')) ??
    (optimizationInterrupted ? '已中断' : optimizationRunning ? '等待执行' : '结果就绪');
  const latestUpdate =
    translateOptimizationText(readProgressText(progressSummary, 'latest_update')) ??
    translateOptimizationText(readProgressText(progressResult, 'latest_update')) ??
  ...
  const latestCandidateLabel =
    translateOptimizationText(readProgressText(progressSummary, 'latest_candidate_label')) ??
    translateOptimizationText(job?.result.best_candidate_label) ??
    translateOptimizationText(selectedCandidate?.label) ??
    null;
  const hasCandidatePanel = !optimizationProgressState && candidateRows.length > 0;
  ...
  const selectedHeatmap = useMemo(
    () =>
      selectedCandidate
        ? decorateHeatmapWithMetrics(selectedCandidate.analysis?.heatmap, selectedCandidate.metrics ?? {})
        : null,
    [selectedCandidate],
  );
  const hasHeatmap = Boolean(!optimizationProgressState && (selectedHeatmap?.cells?.length ?? 0) > 0);
  const hasValidation = Boolean(
    !optimizationProgressState && (selectedCandidate?.analysis?.validation_windows?.length ?? 0) > 0,
  );
  const hasCandidateShelf = Boolean(!optimizationProgressState && candidateRows.length > 0);
  const canPromoteSelectedCandidate = Boolean(selectedCandidate && selectedCandidate.display_kind !== 'baseline');
  const promoteButtonLabel = selectedCandidate?.display_kind === 'baseline' ? '当前基准不可晋升' : TEXT.promoteVersion;
  const strategyDisplayName = formatVersionedStrategyName(
    translateOptimizationText(strategy?.name) ?? job?.strategy_id ?? TEXT.resultsTitle,
    strategy?.current_parameter_version,
  );
  const heroTitle = `参数优化：${strategyDisplayName}`;
  const heroElapsedSeconds = calculateElapsedSeconds(job?.created_at, job?.completed_at ?? job?.updated_at);
  const heroSummary = formatOptimizationHeroSummary(optimizationCombinationCount, heroElapsedSeconds);
  const heroCopy =
    optimizationProgressState
      ? latestUpdate
      : heroSummary ??
  ...
  async function handlePromote(): Promise<void> {
    ...
    try {
      ...
      setJob(refreshedJob);
      setSelectedCandidateId(refreshedJob.result.best_candidate_id ?? refreshedJob.candidates[0]?.id ?? selectedCandidate.id);
      setNotice(`已完成版本晋升，策略已更新为 ${formatVersionedStrategyName(refreshedStrategy.name, refreshedStrategy.current_parameter_version)}。`);
    } catch (caught) {
      setError((caught as Error).message);
    } finally {
      setSaving(false);
  ...
  async function handleResume(): Promise<void> {
    ...
    try {
      ...
      setError(null);
      setNotice(null);
      const resumed = await api.resumeOptimizationJob(job.id, `resume-${job.id}-${nextTrialIndex}`);
      ...
      setJob(resumed);
      setSelectedCandidateId(resumed.result.best_candidate_id ?? resumed.candidates[0]?.id ?? null);
    ...
    <div className="optimization-lab-page">
      ...
      <section className="optimization-lab-panel optimization-lab-panel--hero">
          ...
          {job ? (
            <div className="optimization-meta-chips">
              ...
              {optimizationRangeSummary ? (
                <span className="status-chip status-chip--soft optimization-range-chip">参数组合：{optimizationRangeSummary}</span>
              ) : null}
            </div>
          ) : null}
        ...
        <div className="optimization-hero-actions optimization-hero-actions--single-row">
          ...
          {optimizationInterrupted ? (
            <button className="primary-button" disabled={saving || !resumeReady} onClick={() => void handleResume()} type="button">
      ...
      {!loading && job ? (
        <>
          {optimizationProgressState ? (
            <section className="optimization-lab-panel optimization-progress-panel" style={{ display: 'grid', gap: '22px' }}>
              <div className="optimization-lab-panel__heading">
                <div>
                  <p className="optimization-lab-eyebrow">实时进度</p>
                  <h2>{optimizationInterrupted ? '优化已中断' : '优化进行中'}</h2>
                </div>
              ...
              <div className="optimization-metric-row">
                <article className="optimization-metric-tile" style={{ display: 'grid', gap: '10px', padding: '16px 18px' }}>
                  <span>已完成组合</span>
                ...
                <article className="optimization-metric-tile" style={{ display: 'grid', gap: '10px', padding: '16px 18px' }}>
                  <span>预计完成</span>
                  <strong>{formatExpectedCompletion(estimatedCompletedAt)}</strong>
                </article>
              </div>
              <div className="optimization-summary-list">
                <div
                  className="optimization-summary-row"
                  style={{ display: 'grid', gridTemplateColumns: 'minmax(108px, 136px) minmax(0, 1fr)', alignItems: 'start', gap: '18px', padding: '0 0 14px' }}
                >
                  <span>最新更新</span>
                  <strong style={{ textAlign: 'left', lineHeight: '1.72' }}>{latestUpdate}</strong>
                </div>
                <div
                  className="optimization-summary-row"
                  style={{ display: 'grid', gridTemplateColumns: 'minmax(108px, 136px) minmax(0, 1fr)', alignItems: 'start', gap: '18px', padding: '14px 0' }}
                >
                  <span>最佳候选</span>
                  <strong style={{ textAlign: 'left', lineHeight: '1.72' }}>{bestMetricsSummary ?? latestCandidateLabel ?? '等待首批样本'}</strong>
                </div>
                <div
                  className="optimization-summary-row"
                  style={{ display: 'grid', gridTemplateColumns: 'minmax(108px, 136px) minmax(0, 1fr)', alignItems: 'start', gap: '18px', padding: '14px 0 0' }}
                >
                  <span>{optimizationInterrupted ? '恢复起点' : '下一试验'}</span>
                  <strong style={{ textAlign: 'left', lineHeight: '1.72' }}>
                    {optimizationInterrupted && interruptedReason ? `${nextActionLabel} · ${interruptedReason}` : nextActionLabel}
                  </strong>
                </div>
              </div>
            </section>
                    ...
                    <section className="optimization-lab-panel optimization-results-card">
                      <div className="optimization-lab-panel__heading">
                        <div>
                          ...
                          <p className="optimization-panel-subtitle">{PANEL_SUBTITLES.candidatePanel}</p>
                        </div>
                      </div>
                      <div className="optimization-lab-table-shell">
                        <table className="optimization-lab-table">
                            ...
                            {candidateRows.map((candidate) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td>
                                    <div className="optimization-lab-table__stack">
                                      <strong>{getCandidateDisplayText(candidate.title ?? candidate.label)}</strong>
                                    </div>
                                  </td>
                                  <td className="optimization-lab-table__cell--wrap">
                                    <div className="optimization-parameter-summary">
                                      {buildRangeParameterEntries(candidate.parameter_snapshot, optimizationSearchSpace).length ? (
                                        ...
                                        <span>{buildCandidateParameterSummary(candidate.parameter_snapshot, optimizationSearchSpace) || '-'}</span>
                                      )}
                                    </div>
                                  </td>
                                  <td>{formatReturnRate(getCandidateMetric(candidate, 'annualized_return') ?? getCandidateMetric(candidate, 'cagr'))}</td>
                                  ...
                                  <td>{translateOptimizationText(candidate.status_label) ?? formatOptimizationStatus(candidate.status)}</td>
                                </tr>
                              );
                            })}
                          </tbody>
                    ...
                    <section className="optimization-lab-panel optimization-results-card optimization-results-card--rail">
                      <div className="optimization-lab-panel__heading">
                        <div>
                          ...
                          <p className="optimization-panel-subtitle">{PANEL_SUBTITLES.stabilityCenter}</p>
                        </div>
                      </div>
                      ...
                      <div className="optimization-metric-row">
                        <article className="optimization-metric-tile">
                          <span>年化收益率</span>
                          <strong>{formatReturnRate(getCandidateMetric(selectedCandidate, 'annualized_return') ?? getCandidateMetric(selectedCandidate, 'cagr'))}</strong>
                        </article>
                        <article className="optimization-metric-tile">
                          <span>收益夏普</span>
                      ...
                      <section className="optimization-evaluation-panel">
                        <div className="optimization-parameter-panel__title">综合评价</div>
                        <div className="optimization-evaluation-panel__headline">
                          <strong>综合得分 {formatMetric(selectedCandidate.score, 3)}</strong>
                          <span>{translateOptimizationText(selectedCandidate.status_label) ?? '观察中'}</span>
                        </div>
                        <p>{describeCandidateEvaluation(selectedCandidate)}</p>
                      </section>

                      <p className="optimization-results-summary">
                        {translateOptimizationText(selectedCandidate.analysis?.stability_summary) ??
                          translateOptimizationText(selectedCandidate.analysis?.thesis) ??
                    ...
                    <section className="optimization-lab-panel">
                      <div className="optimization-lab-panel__heading">
                        <div>
                          ...
                          <p className="optimization-panel-subtitle">{PANEL_SUBTITLES.heatmapPanel}</p>
                        </div>
                      </div>
                      <div aria-label="参数热区指标切换" className="optimization-heatmap-toggle" role="radiogroup">
                        {HEATMAP_METRIC_OPTIONS.map((option) => (
                          <button
                            ...
                            className={`optimization-heatmap-toggle__option ${
                              selectedHeatmapMetric === option.key ? 'optimization-heatmap-toggle__option--active' : ''
                            }`}
                            key={option.key}
                      ...
                      <div className="optimization-heatmap-shell">
                        <table className="optimization-heatmap-table">
                          <thead>
                            <tr>
                              <th>{selectedHeatmap ? getDisplayText(selectedHeatmap.y_label, humanizeKey(selectedHeatmap.y_key ?? '')) : '-'}</th>
                              {(selectedHeatmap?.x_values ?? []).map((value) => (
                                <th key={`heatmap-x-${value}`}>{value}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {(selectedHeatmap?.y_values ?? []).map((rowValue) => (
                              <tr key={`heatmap-row-${rowValue}`}>
                                <th>{rowValue}</th>
                                {(selectedHeatmap?.x_values ?? []).map((columnValue) => {
                                  const cell = (selectedHeatmap?.cells ?? []).find(
                                    (item) => item.x === columnValue && item.y === rowValue,
                                  );
                                  return (
                                    <td
                                      className={`optimization-heatmap-cell optimization-heatmap-cell--${
                                        cell ? resolveHeatmapTone(selectedHeatmap?.cells ?? [], selectedHeatmapMetric, cell) : 'cool'
                                      } ${
                                    ...
                                    >
                                      {formatHeatmapMetricValue(
                                        ...
                                        getHeatmapCellMetricValue(cell, selectedHeatmapMetric),
                                      )}
                                    </td>
                                  );
                                })}
                    ...
                    <section className="optimization-lab-panel">
                      <div className="optimization-lab-panel__heading">
                        <div>
                          ...
                          <p className="optimization-panel-subtitle">{PANEL_SUBTITLES.validationPanel}</p>
                        </div>
                      </div>
                      <div className="optimization-lab-table-shell">
                          ...
                          <tbody>
                            {(selectedCandidate.analysis?.validation_windows ?? []).map((windowItem) => (
                              <tr key={windowItem.label}>
                                ...
                                <td>{formatReturnRate(windowItem.annualized_return)}</td>
                      ...
                      <p className="optimization-validation-footnote">{VALIDATION_WINDOW_GUIDANCE}</p>
                    </section>
                  ) : null}
                </div>
              ) : null}
                ...
                <section className="optimization-lab-panel">
                  ...
                  <div className="optimization-shelf-grid">
                    {candidateRows.map((candidate) => (
                      ...
                      >
                    <strong>{getCandidateDisplayText(candidate.title ?? candidate.label)}</strong>
                        <span>{translateOptimizationText(candidate.status_label) ?? formatOptimizationStatus(candidate.status)}</span>
                        <p>{translateOptimizationText(candidate.analysis?.shelf_copy) ?? translateOptimizationText(candidate.summary) ?? '-'}</p>
                      </button>
                    ))}
                  </div>
                </section>
