...
import type { ApiBacktestRunDetail, ApiOptimizationJobDetail, ApiStrategyDetail, DemoApi } from './types';
...
function createInterruptedJob(): ApiOptimizationJobDetail {
  return {
    ...
    summary: {
      ...
      latest_update: 'Progress preserved at 11/70. Click Continue Optimization to resume.',
    ...
    result: {
      ...
      summary: 'Progress preserved at 11/70. Click Continue Optimization to resume.',
      status: 'INTERRUPTED',
      progress_pct: 16,
      current_stage: 'Interrupted at 11/70',
      latest_update: 'Progress preserved at 11/70. Click Continue Optimization to resume.',
...
describe('optimization results progress state', () => {
  it('renders paused jobs inside the optimization in-progress panel and supports resume', async () => {
    ...
    await waitFor(() => expect((currentApi.resumeOptimizationJob as ReturnType<typeof vi.fn>).mock.calls.length).toBe(1));
    expect((currentApi.resumeOptimizationJob as ReturnType<typeof vi.fn>).mock.calls[0]?.[0]).toBe('opt-paused-001');
    expect((currentApi.resumeOptimizationJob as ReturnType<typeof vi.fn>).mock.calls[0]?.[1]).toBe('resume-opt-paused-001-12');
    ...
    expect(container.textContent).not.toContain('Resuming optimization from trial 12.');
    expect(container.querySelector('.optimization-inline-notice')?.textContent).toContain('已继续优化');
  });
  ...
  it('does not show a resumed notice when the job remains interrupted', async () => {
    ...
    expect(container.querySelector('.error-banner')?.textContent).toContain('继续优化未启动');
  });
});
