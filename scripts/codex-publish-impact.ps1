...
function Invoke-ImpactGate {
    ...
    $arguments = @('-ExecutionPolicy', 'Bypass', '-File', $impactScript, '-Scope', $Scope, '-Remote', $Remote)
    if (-not [string]::IsNullOrWhiteSpace($SinceLastValidated)) {
        $arguments += @('-SinceLastValidated', $SinceLastValidated)
    }
    if ($SkipFetch) {
        $arguments += '-SkipFetch'
    }
    ...
    Write-Host "Running impact gate with scope=$Scope." -ForegroundColor Cyan
    & powershell @arguments
    if ($LASTEXITCODE -ne 0) {
        throw "Impact gate failed with exit code $LASTEXITCODE"
    }
}
...
function Get-GateField {
    ...
    $summaryPath = Join-Path $repoRoot 'harness\reports\smoke\latest-impact-gate.md'
...
function Write-PushAttestation {
    ...
    $lines = @(
        ...
        ('- impact_scope: ' + (Get-GateField -Name 'scope')),
        ('- impact_head_sha: ' + (Get-GateField -Name 'head_sha')),
        ('- impact_base_sha: ' + (Get-GateField -Name 'base_sha')),
        ('- impact_content_fingerprint: ' + (Get-GateField -Name 'validation_content_fingerprint')),
        ('- metadata_attempts: ' + $MetadataAttempts)
    )
    $lines | Set-Content -LiteralPath $attestationPath -Encoding utf8
    Write-Host "Wrote push attestation: $attestationPath" -ForegroundColor Green
}
...
function Invoke-PushWithMetadataLoop {
    ...
    while ($true) {
        ...
        Write-Host 'Refreshing committed impact evidence after metadata snapshot.' -ForegroundColor Cyan
        Invoke-ImpactGate -Scope 'Committed'
    }
}

if (-not (Test-Path -LiteralPath $impactScript)) {
    throw "Impact validation script is missing: $impactScript"
}
...
if ($SkipCommit) {
    ...
    if ($stagedPaths.Count -eq 0 -and -not $PlanOnly) {
        throw 'No staged files found. Stage the exact publish set before running codex-publish-impact.'
    }
...
if ($PlanOnly) {
    Write-Host 'PlanOnly complete; no impact validation, commit, or push was run.' -ForegroundColor Cyan
    exit 0
}
