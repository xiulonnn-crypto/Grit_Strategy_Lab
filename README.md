...
Grit Strategy Lab 的本地策略研究与回测工作台。当前仓库包含可运行的 FastAPI 后端、React/Vite 前端，以及一条可用的主链路：`workspace -> creation -> backtest -> run detail -> optimization`。
...
更多启动、测试、恢复和推送细节请直接看下面的文档与脚本索引，不再在首页重复展开。
...
- [scripts/codex-smoke.ps1](./scripts/codex-smoke.ps1): 一键跑固定 smoke 链路
