...
- Factor mining supports `HYBRID_COMPOSITION` as an additive generation mode for the same factory. The mode emits deterministic template candidates plus bounded cross-family pairwise exploration from existing active factors, records `source_factor_ids`, `recipe_kind`, `recipe_family`, `orthogonality_intent`, and composition policy metadata in mining summaries/candidates, then sends candidates only as far as D2 quarantine. Composition lineage is stored through `factor_lineage_edges` from parent factors to quarantine candidates and, after manual publish, to the resulting `AUTO_MINED` factor; no composition task writes directly to `factor_definitions`.
