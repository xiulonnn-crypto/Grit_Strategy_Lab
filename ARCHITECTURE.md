...
- PRUNE 冗余裁剪采用“真实可复核证据优先”的治理口径：同簇候选、MVP 和执行前重算仍共享治理概览中预览增强后的线上因子宇宙，但裁剪触发必须来自实测证据，例如对齐日期后的诊断 `ic_series` Pearson 相关性，且样本重叠不少于 6 个观测、绝对相关性大于 `0.90`。因子库热力图、descriptor 启发式、表达式同族判断和 cluster top-N 只能作为 warning/审计线索，不能单独写入 `PRUNED`。服务端执行时必须重新计算实测证据，不能只信前端 payload；历史 `PRUNED` 因子可通过 `/factor-governance/prune-recovery/preview` 与 `/factor-governance/prune-recovery/apply` 回溯，只有缺少高于阈值的实测/检疫冗余证据时才恢复为 `VERIFIED`，并写入 `factor_governance_events`。
