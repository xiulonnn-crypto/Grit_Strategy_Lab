...
- `data_trust_summary.layers[]` 固定按角色聚合：价格主链、成分股历史、退市/身份、公司行动/zero-event、长周期补丁、精修来源。`Tiingo` 是 EOD OHLCV 与公司行动修复首选；`FMP historical constituent` 证明历史成员 in/out 日期；`SEC EDGAR/CIK` 只证明身份和生命周期，不提供价格且停止申报不能直接等同破产；`Stooq` 与 Kaggle 是 price-only 补丁，不能单独清公司行动或身份门禁；`Polygon` 只在 key 存在时作为关键缺口精修。
...
- 因子治理投影把后端细粒度状态映射为前台四类语义：`robust/稳健`、`needs_calibration/待校准`、`decayed/失效` 与 `sandbox/沙箱`。相关性高、同族重叠、IC/IR 不稳定、coverage 边缘、换手衰减和诊断过期属于策略创建风险提示；PIT 缺口、未来函数、不可回放/current-only 字段、unsafe expression、缺失 `available_at`、启用中性化但缺行业 PIT 才是硬阻断。
- 因子库列表允许研究态指标投影，但必须标清来源：没有真实诊断的人工因子或新增默认因子只能复制前五个基准默认因子的最近诊断摘要并标记 `REFERENCE_ONLY`、`data_lineage.kind=REFERENCE_DEFAULT_FACTOR`，不得写入 `last_diagnostic_run_id` 或让用户误以为已经跑过正式诊断。`VERIFIED_PIT_WINDOW_INCOMPLETE` 只表示完整 Verified 窗口仍未满足，在 Sandbox/Limited 研究态属于风险提示；`PRICE_SNAPSHOT_NOT_READY`、`PIT_GATE_BLOCKED`、`UNIVERSE_HISTORY_BLOCKED`、基础面/行业 PIT 缺口、未来函数和不可回放字段仍保持硬阻断。
...
- 第二步在现有响应上追加可选 view-model：`multi_factor_profile` 从策略参数、因子库元数据、PIT 覆盖和诊断状态构建；`multi_factor_precheck` 在回测 preview 与 submit 前构建并作为提交门禁；`multi_factor_attribution` 从回测运行参数、因子得分、持仓/收益摘要生成，并用 `attribution_source` 标明完整归因或估算归因；`multi_factor_parameter_ranges` 把因子权重、打分方法、再平衡频率和行业中性化映射为现有优化参数范围。
...
- 运行中的市场数据来源链按角色分工：PIT 缺口修复优先级是 `Tiingo -> FMP -> Stooq/Kaggle -> SEC/CIK -> Polygon`。价格热路径仍可使用 Yahoo/yfinance 等公开源，但 Full Ready 修复必须把 Tiingo 视为 EOD 与公司行动首选、FMP 视为成分股和退市身份增强、Stooq/Kaggle 视为 price-only 长周期补丁、SEC EDGAR/CIK 视为身份生命周期确权、Polygon 视为 key-gated 精修 lane。
...
- 左侧导航当前分为 `组合`、`策略`、`因子`、`数据` 四组；组合组包含 `组合仪表板`、组合列表、组合回测、组合实验室与 `资产库`，策略组保留既有主链路并把 `workspace` 对外标签统一为 `策略工作台`，因子组包含正式入口 `因子库`、已接通任务视图的 `挖掘沙盒` 以及仍为后续占位的 `隔离检疫区`，数据组保留 `PIT 清洗中心` 与 `数据快照`。`#/snapshots` 页面结构本期不调整，只作为数据基座入口保留。
