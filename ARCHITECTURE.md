...
- 左侧导航当前分为 `组合`、`策略`、`因子`、`数据` 四组；组合组包含 `组合仪表板`、组合列表、组合回测、组合实验室与 `资产库`，策略组保留既有主链路并把 `workspace` 对外标签统一为 `策略工作台`，因子组包含正式入口 `因子库`、已接通任务视图的 `挖掘沙盒` 以及 D2 `检疫工作台`，数据组保留 `PIT 清洗中心` 与 `数据快照`。`#/snapshots` 页面结构本期不调整，只作为数据基座入口保留。
