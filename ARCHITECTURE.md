...
- Raw_F2 publish is blocked unless WNZT evidence is complete, quarantine status is `PASSED`, `publish_status` is `ELIGIBLE`, and the candidate carries both `operator_config_snapshot_id` and `f1_catalog_snapshot_id`.
- `GET /factor-factory/overview` projects front-stage monitors from existing JSON read models: formula count, initial screen pass count, quarantine pass count, S-grade promotion count, alpha concentration, and rejection reason distribution.
