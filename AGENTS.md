...
- If `#/factors/imports` looks stale after a factor import is submitted for review, first run `powershell -ExecutionPolicy Bypass -File .\scripts\codex-probe-factor-imports-refresh.ps1 -Json -ExpectSubmitted` after the runtime preflight. For source-manifest prechecks such as AQR QMJ, use `-ExpectPendingPrecheck -SourceId aqr -DatasetKey aqr_public_style_factors`. Compare the latest external import job from `/factor-factory/overview` with `/factor-sources/import-jobs/{id}` and the page's browser request log; the route must request overview plus job detail, not only `/factor-sources/registry`.
