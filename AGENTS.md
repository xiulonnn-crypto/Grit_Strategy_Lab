...
- When snapshot provider keys such as `KAGGLE_API_TOKEN`, `POLYGON_API_KEY`, or `NASDAQ_DATA_LINK_API_KEY` are written from the UI or manually, do not rerun plain `QuickStart-Grit.ps1` and expect a healthy backend to reload them. Use `QuickStart-Grit.ps1 -ForceRestart -RestartReason "snapshot provider credentials updated"` so supervisor records the forced restart and repo-owned `8000/4173` listeners are replaced.
...
- Factor quarantine intake must mirror the mining sandbox top-candidate projection, not the full internal mining candidate ledger. The workbench queue must dedupe by normalized expression, and PIT evidence that is not Full Ready should enter review/observation unless another hard gate fails; it must not auto-publish.
