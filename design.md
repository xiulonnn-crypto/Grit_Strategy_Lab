...
- Sidebar 导航组固定为 `组合 / 策略 / 因子 / 数据`。因子组当前包含 `因子库`、`挖掘沙盒` 与 `检疫工作台`；`因子库`、`挖掘沙盒` 和 `检疫工作台` 都是正式可操作入口，详情、编辑器和多因子策略创建从因子库或治理动作进入。数据组固定保留 `PIT 清洗中心` 与 `数据快照`，`#/snapshots` 页面结构本期不调整。
...
| `#/factors/sandbox` | hero + 4 指标 + 任务队列 + 候选摘要 + 创建任务 | 承载 D1 挖掘沙盒；候选表达式只做研究摘要，不直接进入正式因子库 |
| `#/factor-models/new` | hero + 因子选择篮子 + 权重/方向 + 打分预览 + 策略门禁 | 承载 F1 多因子策略创建，支持行业中性化配置；创建成功后进入既有策略详情页 |
| `#/factors/quarantine` | 检疫工作台标题 + 候选队列 + 检疫报告 + 发布审计 + 拒绝样本库 | 承载 D2 检疫闭环；候选来自挖掘沙盒，必须经过 PIT、样本外、正交化、去重和发布资格门禁后才能发布 |
...
- **因子库**：属于 `因子` 组的常驻入口，使用紧凑标题、4 个摘要指标、筛选工具栏和 dense table 展示默认因子、来源、标签、诊断状态、PIT 覆盖、最近诊断和操作；表格主标题显示中文 Display Name，副标题显示 canonical ID，并支持来源前缀、类别和处理算子筛选；不得使用广场类命名。三期只允许把指标区第四张卡改为「治理队列」，点击后打开治理动作弹层；工具栏、表格、相关性热力图和既有资源/诊断模块不随本轮改版。最近诊断缩略图必须带 0 位虚线，正 IC 区域使用浅绿、负 IC 区域使用浅红，用于快速识别方向反转。操作列提供最多两个因子的「比对」勾选，勾选两个因子后下方研判区切换为 IC 走势和多头超额收益的因子指纹对比。来源显示必须本地化为“系统默认 / 人工 / 自动挖掘”，不得暴露 raw enum、`s/m/a` 前缀或后端字段名。
- **挖掘沙盒**：`#/factors/sandbox` 使用 V1.7 1960px 稿的研究工作台节奏，顶部 hero 后接 4 个指标、任务队列、候选摘要与创建任务表单；任务队列和候选摘要先按同配置/同表达式去重，默认最多露出 3 条，更多内容在模块内滚动查看。表单字段包含 Universe、时间窗、算子集合、候选数量、随机种子和 IC 门槛。风险提示必须明确候选表达式不会直接进入正式因子库，页面不提供晋升按钮。
- **多因子策略创建**：`#/factor-models/new` 使用三栏工作台，左侧选择系统或验证因子，中间配置权重、方向、再平衡频率和行业中性化，右侧展示打分预览、PIT 覆盖、预计换手和物化门禁。治理队列跳转只能预填因子、方向和建议权重，并以草稿/待审查语气提示；启用行业中性化但缺少 PIT 行业字段时，门禁必须显示未执行 blocker，并禁用创建。
- **检疫工作台**：`#/factors/quarantine` 是 D2 正式工作台，沿用因子二期的研究台风格，首屏必须展示候选队列、检疫报告、发布审计和拒绝样本库四个模块。候选不得直接进入因子库，研究态 PIT 豁免只进入观察或待复核，不出现自动覆盖生产策略的动作。
