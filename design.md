...
- Sidebar 导航组固定为 `组合 / 策略 / 因子 / 数据`。因子组当前包含 `因子库`、`挖掘沙盒` 与 `隔离检疫区`；只有 `因子库` 是一期正式可操作入口，后两者只展示后续里程碑占位，详情和编辑器从因子库动作进入。数据组固定保留 `PIT 清洗中心` 与 `数据快照`，`#/snapshots` 页面结构本期不调整。
...
| `#/factors` | 紧凑标题 + 筛选工具栏 + 因子表格 + 语义聚类热力区 | 对外名称为“因子库”，展示默认因子、诊断状态、必要动作、相关性聚类和双因子指纹比对 |
| `#/factors/new` | 紧凑标题 + 因子编辑表单 + 诊断预览 | 用类 Excel 公式录入人工因子，保存前可查看诊断预览 |
| `#/factors/sandbox` | 占位标题 + 后续里程碑说明 | 当前只保留导航与路线，不承载自动挖掘工作流 |
...
- **默认因子文案**：五个默认因子名称固定为 `12-1月截面动量排名`、`滚动市盈率倒数 (LTM)`、`252日年化波动率排名`、`即时对数总市值` 与 `自由现金流收益率 (TTM)`；副标题 canonical ID 固定为 `s_mom_12m1m_rank`、`s_val_ep_ltm_raw`、`s_vol_252d_rank`、`s_size_cur_log` 与 `s_qlty_fcfy_ttm_raw`。页面文案全部中文，必要英文缩写只作为金融术语保留。
