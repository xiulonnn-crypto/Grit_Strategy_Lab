...
- Sidebar 导航组固定为 `组合 / 策略 / 因子 / 数据`。因子组当前只展示 `因子库` 与 `因子工厂`；`因子工厂` 合并原 `挖掘沙盒` 与 `检疫工作台`，旧 `#/factors/sandbox` 和 `#/factors/quarantine` 作为兼容入口进入同一页对应分区。详情、编辑器和多因子策略创建从因子库或治理动作进入。数据组固定保留 `PIT 清洗中心` 与 `数据快照`；本期数据页升级为“快照分层治理 + PIT 门禁联动”，其中 `#/snapshots?tab=equity` 保留线上股票 tab 工作台骨架并引入 L1-L4 数据层级，`#/pit-data` 保留既有清洗链路并新增 L1-L4 PIT 准入与因子维度就绪矩阵。
...
| `#/pit-data` | 紧凑标题 + L1-L4 PIT 准入卡 + 因子诊断准入矩阵 + 覆盖率下钻 + 规则预览 + 门禁行动列表 | 数据组的一期入口，服务因子诊断；首屏保留研究豁免、覆盖率下钻、规则工作站和门禁行动列表，并用中文状态词展示 `已验证 / 沙箱观察 / 阻断 / 停用`。价格阻塞可跳转并高亮数据快照；研究态豁免只能进入 observation/review，不得在新矩阵中被洗白为已验证 |
...
| `#/factors/factory` | hero + 自动化状态 + 因子漏斗 + 当前 run 挖掘队列 + 自动送检/自动检疫 + 检疫发布队列 + 拒绝原因列 + PIT 诊断证据 | 承载 D1/D2 因子工厂；启动自动化为每日 `GMT+8 14:00`，立即运行为一次性 run；候选必须自动进入 D2 检疫并执行检疫，不直接进入正式因子库 |
...
| `#/snapshots` | heading / tabs + 股票分层治理工作站或债券治理工作站 | 股票 tab 保留线上健康仪表盘、`查看明细` 弹层、`刷新股票快照` 和底部凭据 rail，仅把前四张总览卡替换为 `L1 基础行情 / L2 财务截面 / L3 分析师与情绪 / L4 宏观与衍生品`，第五张保留 `最新刷新（EST）`。股票页中段新增分层工作站、异常告警区、因子维度就绪矩阵与原始快照台账；数据可信层继续放在最下方，并使用独立的数据源证据卡片表达。卡片文案使用简洁中文金融口径，按价格主链、成分历史、长周期补价、身份确权、关键精修解释证据范围与门禁边界，不直接展示 raw trust tier 或英文枚举；缺失 API_KEY / 本机配置统一汇总到底部选择器，选择后只展示对应输入与复制设置加重启命令入口，并明确提示需在启动 QuickStart 的同一个 PowerShell 中执行。输入值只在当前浏览器标签页暂存，刷新保留，关闭标签页或点击清空删除；不提交后端、不落库、不改变 provider 状态。债券 tab 继续连接资产腿创建、质量审计与 raw registry |
...
- **PIT 清洗中心**：属于 `数据` 组的一期入口，显示点时价格、样本池、异常清洗、未来函数门禁和可诊断范围；顶部摘要升级为 `L1-L4 PIT 准入卡`，每张卡都要在状态下方显示一行微缩原因，区分价格源缺口、身份映射挂起、历史 Universe 缺口、`available_at` 缺失、current-only 字段或清洗规则过严。覆盖率数字旁必须提供「下钻分析」，展示缺失标的分桶、样例和下一步动作；每个缺口卡片必须同时显示数量占比、市值权重占比、时间轴分布和样例标的。身份映射待解析标的使用可点击 chip 打开「身份映射覆盖」弹窗，弹窗展示历史代码路径、标准代码输入和审计原因。页面顶部应展示「数据运维指令」，把当前核心成员缺口、身份映射挂起数量和建议运维动作直接暴露给 operator；点击「重启身份修复任务」后必须出现明确的「执行重启任务」主按钮，执行后展示尝试、成功、失败和剩余待解析数量，避免只停留在说明面板。PIT 清洗中心首屏不展示「Full Ready 免费源修复队列」「补源优先级与证据层」或「外部缓存与精修就绪度」模块；补源队列、证据层和外部缓存精修仍属于数据快照、脚本或 API 证据面，PIT 页面只承载当前门禁、覆盖率下钻、清洗规则、身份修复和门禁行动。清洗规则工作台使用 MAD / 3σ / 分行业阈值的「阈值预演」，展示预估剔除比例、样本数和异常点；READY 或研究就绪时展示 Universe 历史成员数量变化图和带时间轴/复权因子的复权校验轨迹；首屏新增「因子诊断准入矩阵」与 `snapshot -> PIT` 逻辑映射区，用中文状态词展示 `已验证 / 沙箱观察 / 阻断 / 停用`，并明确哪些 snapshot check 点亮哪些因子算子。门禁清单中的阻塞码是行动链接，跳到 `#/snapshots?tab=equity&target=...` 并高亮目标行；研究态豁免要用明显 banner 告知不能晋升，并展示被忽略项的潜在 IC 扰动和市值权重估算。它不替换 `#/snapshots` 的股票/债券快照结构，左侧导航仍需保留 `数据快照`。
- **因子库**：属于 `因子` 组的常驻入口，使用紧凑标题、4 个摘要指标、线上/已下线 tab、筛选工具栏和 dense table；线上 tab 展示因子、来源、诊断状态、最近诊断、因子级别、最近更新和比对 / 操作，已下线 tab 展示 `DEPRECATED/PRUNED` 因子并保留下线理由与时间，不再单列“阻断 / 风险”。诊断状态按钮弹层承载主要风险、硬阻断说明和管理动作，避免把同一判断拆成两列。表格主标题显示中文 Display Name，右侧显示中性灰色标准分类 tag，副标题显示 canonical ID，并支持来源前缀、标准类别和处理算子筛选；标准类别只按动量、规模、估值、质量、风险、情绪六类展示，`beta/vol` 映射为风险，`inv/qlty` 映射为质量，`liq` 映射为情绪，六类之外的 composite/alpha 显示为“其他”；不得使用广场类命名。指标区第四张卡为「治理任务」，点击后打开治理动作弹层；弹层只展示 `DEPRECATE`、`PRUNE` 和策略草稿任务，不展示历史复核、拥挤或退化消息；执行类任务必须展示指令、触发条件、关联因子、理由和二次确认操作，策略草稿任务只跳转多因子创建页并保持草稿。最近诊断缩略图必须带 0 位虚线，正 IC 区域使用浅绿、负 IC 区域使用浅红，用于快速识别方向反转。因子单元格内提供最多两个因子的「比对」勾选和详情入口，勾选两个因子后下方研判区切换为 IC 走势和多头超额收益的因子指纹比对。来源显示必须本地化为“系统默认 / 人工 / 自动挖掘”，不得暴露 raw enum、`s/m/a` 前缀或后端字段名。
- **因子诊断状态筛选**：因子库诊断状态固定为“稳健 / 待校准 / 沙箱 / 失效”四档；稳健要求 S/A/B、覆盖率大于 90%、IR 稳定且分组收益单调良好，且无风险提示或硬阻断；待校准用于覆盖不足、相关性拥挤、近期 IC 衰减、10Y 补源队列等降权研究态；沙箱用于无 IC/IR 的预览态；失效用于 C/D 或分层收益倒挂。表格筛选与诊断弹层必须复用同一管理动作文案。
- **因子工厂**：`#/factors/factory` 使用 V1.7 1960px 稿的研究工作台节奏，把挖掘沙盒与检疫工作台收敛为一个生产页面。顶部 hero 后接自动化状态条、启动自动化 / 暂停自动化 / 立即运行按钮和因子漏斗；三栏工作台展示自动化 run、当前 run 挖掘队列与检疫发布队列。每日自动化固定为 `GMT+8 14:00`，工厂 run 完成挖掘后必须以完整 Raw_F2 批次作为检疫来源，top candidates 只作为前台预览或分页默认范围；手动“送入检疫”也必须执行 D2 检疫并选中结果。任务队列和候选摘要先按当前 `mining_job_id` 与同表达式去重，已进入检疫队列的候选必须从左侧候选摘要隐藏，默认露出精简队列，更多内容在模块内滚动查看。检疫发布队列中 `REJECTED` 候选必须展示独立“拒绝原因”列且使用中文硬阻断原因；Auto-Residual 与回撤硬约束不再作为独立下方模块展示，其证据进入检疫结果、拒绝原因、审计与风险提示。L1 只展示未经算子的事实字段，`Return(Close, n)`、`MA(...)`、`Std(...)` 等含算子候选必须展示为 L2 Raw Signal，并在准入规则、详情和风险提示中暴露 WNZT 缺失与同族动量冗余观察。卡片 IR 标签必须使用 Newey-West 重叠持有期修正后的 IR。风险提示必须明确候选表达式不会直接进入正式因子库，只有 D2 `PASSED + ELIGIBLE` 可发布；PIT 非 Full Ready 不再单独制造拒绝态，也不得拼进拒绝原因，但必须进入诊断、因子级别、发布审计和风险提示。
...
- 设计锁定页面是否在交付前声明并验证几何断言：模块高度、容器宽度、表格列宽、右侧空白、文本与图表重叠、按钮折行、滚动条、sticky/浮层位置、弹层初始/打开/关闭状态。
- 设计稿中的条件模块是否被真实触发后验收，例如可发布名单、非空列表、空态、告警态、失败态、弹层展开态；不能因为当前 API 返回 0 条就跳过设计稿里明确存在的模块。
- Trace Matrix 是否同时包含设计截图与 live 截图，并逐项比较模块数量、顺序、卡片数、标题、字段、按钮、密度和允许偏离项；仅有 DOM 几何或功能断言不能声明“100% 一致”。
- UI review 是否先建立矩阵 skeleton，再一次性采集 design/live desktop、mobile、关键弹层/抽屉/条件态截图、metrics JSON、overflow report、console errors 和 failed requests；不要在没有矩阵的情况下反复手动截图。
- 是否把验收拆成结构、几何、内容三层并行检查：结构核对模块和控件数量，几何核对尺寸/溢出/滚动/sticky，内容核对中文文案、状态、真实数据映射和 raw backend leak。
- 是否在矩阵开头声明 shared shell 是否属于本次验收范围，并标记字段是 `frozen-design`、`live-substitutable` 还是 `must-match-copy`，避免把真实数据替代或 shell 差异误判成页面主体失败。
- 最终截图是否已被实际打开检查；截图文件存在但未查看，不能算作视觉验收。
- 复合页面是否逐项核对内容架构：区块标题、卡片数量、状态芯片、字段标签和值、KPI、主次按钮、空态、错误态、滚动/sticky 和 responsive。
- 因子工厂等治理页面的 Trace Matrix 是否标明每个数字、列表和状态标签来自 preview、batch、quarantine run 还是 publishable final list；设计文案不得把 legacy top-candidate 预览误写成检疫事实源。
