...
- Sidebar 导航组固定为 `组合 / 策略 / 因子 / 数据`。因子组当前包含 `因子库`、`挖掘沙盒` 与 `隔离检疫区`；`因子库` 和 `挖掘沙盒` 是第二期第一步正式可操作入口，`隔离检疫区` 仍只展示后续里程碑占位，详情和编辑器从因子库动作进入。数据组固定保留 `PIT 清洗中心` 与 `数据快照`，`#/snapshots` 页面结构本期不调整。
...
| `#/factors` | 紧凑标题 + 4 指标 + 筛选工具栏 + 因子 dense table + 语义聚类热力区 | 对外名称为“因子库”，基于线上表格展示七个默认因子、诊断状态、PIT 覆盖、必要动作、相关性聚类和双因子指纹比对 |
| `#/factors/new` | 紧凑标题 + 因子编辑表单 + 诊断预览 | 用类 Excel 公式录入人工因子，保存前可查看诊断预览 |
| `#/factors/sandbox` | hero + 4 指标 + 任务队列 + Top candidates + 创建任务 | 承载 D1 挖掘沙盒；候选表达式只做研究摘要，不直接进入正式因子库 |
| `#/factor-models/new` | hero + 因子选择篮子 + 权重/方向 + 打分预览 + 策略门禁 | 承载 F1 多因子策略创建，支持行业中性化配置；创建成功后进入既有策略详情页 |
| `#/factors/quarantine` | 占位标题 + 后续里程碑说明 | 当前只保留导航与路线，不承载检疫去重或样本外门禁工作流 |
...
- **PIT 清洗中心**：属于 `数据` 组的一期入口，显示点时价格、样本池、异常清洗、未来函数门禁和可诊断范围；顶部状态卡不能只显示 `阻塞`，必须在状态下方显示一行微缩原因，区分价格源缺口、身份映射挂起、历史 Universe 缺口或清洗规则过严。覆盖率数字旁必须提供「下钻分析」，展示缺失 symbol 分桶、样例和下一步动作；每个缺口卡片必须同时显示数量占比、市值权重占比、时间轴分布和样例 symbol。身份映射待解析 symbol 使用可点击 chip 打开 Mapping Overwrite 弹窗，弹窗展示历史 ticker 路径、canonical symbol 输入和审计原因。页面顶部应展示「数据运维指令 (Ops Guidance)」，把当前核心成员缺口、身份映射挂起数量和建议运维动作直接暴露给 operator；点击「重启 Identity Scraper」后必须出现明确的「执行重启任务」主按钮，执行后展示尝试、成功、失败和剩余待解析数量，避免只停留在说明面板。Full Ready 免费源修复队列必须展示剩余 symbol、队列规模、供应商冷却、桶级数量、样例 symbol、alias 候选和拒绝伪 Ready 规则，让 operator 明白下一步是补真实证据而不是豁免。清洗规则工作台使用 MAD / 3σ / 分行业阈值的 what-if 预览，展示预估剔除比例、样本数和异常点；READY 或 Limited Ready 时展示 Universe 历史成员数量变化图和带时间轴/复权因子的复权校验轨迹；门禁清单中的阻塞码是行动链接，跳到 `#/snapshots?tab=equity&target=...` 并高亮目标行；研究态豁免要用明显 banner 告知 `promotion_eligible=false`，并展示被忽略项的潜在 IC 扰动和市值权重估算。它不替换 `#/snapshots` 的股票/债券快照结构，左侧导航仍需保留 `数据快照`。
- **因子库**：属于 `因子` 组的常驻入口，使用紧凑标题、4 个摘要指标、筛选工具栏和 dense table 展示默认因子、来源、标签、诊断状态、PIT 覆盖、最近诊断和操作；表格主标题显示中文 Display Name，副标题显示 canonical ID，并支持来源前缀、类别和处理算子筛选；不得使用广场类命名。最近诊断缩略图必须带 0 位虚线，正 IC 区域使用浅绿、负 IC 区域使用浅红，用于快速识别方向反转。操作列提供最多两个因子的「比对」勾选，勾选两个因子后下方研判区切换为 IC 走势和多头超额收益的因子指纹对比。来源显示必须本地化为“系统默认 / 人工 / 自动挖掘”，不得暴露 raw enum、`s/m/a` 前缀或后端字段名。
- **挖掘沙盒**：`#/factors/sandbox` 使用 V1.7 1960px 稿的研究工作台节奏，顶部 hero 后接 4 个指标、任务队列、Top candidates 与创建任务表单；表单字段包含 Universe、时间窗、算子集合、候选数量、随机种子和 IC 门槛。风险提示必须明确候选表达式不会直接进入正式因子库，页面不提供晋升按钮。
- **多因子策略创建**：`#/factor-models/new` 使用三栏工作台，左侧选择系统或验证因子，中间配置权重、方向、再平衡频率和行业中性化，右侧展示打分预览、PIT 覆盖、预计换手和物化门禁。启用行业中性化但缺少 PIT 行业字段时，门禁必须显示未执行 blocker，并禁用创建。
- **因子占位页**：`隔离检疫区` 只展示后续里程碑说明，不应出现可提交的自动检疫、晋升或生产门禁按钮；它使用同一套紧凑标题和空态语气，避免操作者误以为三期流程已经接通。
- **因子相关性研判**：因子库底部热力图必须按描述符类别自动排序和分区，类别内形成连续矩阵块；高相关阈值默认 `0.7`，同类高相关用蓝色边框提示，跨类别高相关用更强警示边框提示。热力图提供「仅显示高相关对」筛选，并在因子数量增长时启用虚拟矩阵窗口，避免 dense table 与矩阵同时拖慢首屏。
...
- **因子详情/诊断**：页面回答“这个因子能不能用、为什么能用、哪里被数据挡住”。动量和低波因子展示价格 PIT 诊断、IC/IR、换手衰减、风险提示和报告导出；估值、质量和规模因子使用基础面 PIT 种子快照进入诊断流程，缺数据时必须显示具体 `基础面 PIT 缺口` 窗口或字段。
