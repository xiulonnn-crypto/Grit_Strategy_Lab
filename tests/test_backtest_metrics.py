...
def test_run_detail_analysis_prefers_real_trade_events_over_audit_episode_counts():
    ...
    annualized_card = next(card for card in analysis["kpi_cards"] if card["key"] == "annualized_return")
    trade_card = next(card for card in analysis["kpi_cards"] if card["key"] == "trade_count")

    assert analysis["kpi_cards"][1]["key"] == "annualized_return"
    assert len(annualized_card["footer_items"]) == 3
    assert all(isinstance(item["value"], str) and item["value"] for item in annualized_card["footer_items"])
    assert trade_card["primary_text"] == "121"
    assert [item["value"] for item in trade_card["footer_items"]] == ["1", "2"]
...
def test_run_detail_analysis_uses_equity_curve_for_dca_style_strategy_comparison():
    ...
    assert total_return_card["compare_text"] != ""
    assert sharpe_card["compare_text"] != "\u57fa\u51c6: 0.91 | \u6eda\u52a8\u5dee: +0.00 | \u6d4b\u8bd5\u96c6: 0.91"
